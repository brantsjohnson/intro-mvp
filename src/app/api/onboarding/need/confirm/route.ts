...
import { needConfirm, loadOnboardingContext } from "@/lib/onboarding/engine"
...
export async function POST(request: NextRequest) {
  try {
    ...
    const summary = await needConfirm(supabase, ctx)
    return NextResponse.json({ needSummary: summary })
  } catch (err) {
