...
import { fetchOrganizerMatchesSummary } from "@/lib/organizer-metrics"
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const summary = await fetchOrganizerMatchesSummary(supabase, eventId!)
    if (!summary) {
      return NextResponse.json({ error: "Event not found" }, { status: 404 })
    }
    return NextResponse.json(summary)
  } catch (e) {
    console.error("organizer/matches-summary:", e)
