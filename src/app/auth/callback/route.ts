...
export async function GET(request: NextRequest) {
  ...
  // Supabase / OIDC authorization code — must not be confused with encrypted join payloads
  ...
  console.log(
    `[PERF] OAuth successful, user: ${authUser?.id}, total callback time: ${totalTime.toFixed(2)}ms`,
  )
  console.log("Session established:", !!data.session)

  // Invite is kept in sessionStorage (see pending-event-invite.ts) — never append OAuth ?code= here.
